...
namespace JokerMod.Modules.DamageTypes {
    public static class WindMediumType {
        ...
        public static void Init() {
            ...
            Hooks.Handle_HealthComponentTakeDamageProcess_Actions += AddMediumWindDamage;
        }

        private static void AddMediumWindDamage(HealthComponent self, DamageInfo damageInfo) {
            ...
            if (self?.body?.teamComponent?.teamIndex != null && attackerBody?.teamComponent?.teamIndex != null && damageInfo.HasModdedDamageType(damageType)) {
                ...
                if (!sameTeam || isFriendlyFire || isNoTeam) {
                    self.body.AddTimedBuff(SweptDebuff.buffDef, 10f, 1);
                    damageInfo.damage += attackerBody.damage * 5f;
